...
// Setup tab version tracker starts at row 200 (header) and row 201+ (data)
// Moved from rows 73-80 (7 slots) to rows 200-250 (50 slots) — Session 51
...
// Old location constants for one-time migration
...
/**
 * Read the version tracker from the Setup tab (rows 201-250).
 * Auto-migrates from old location (rows 74-80) if new location is empty.
 *
 * @param {string} spreadsheetId
...
export async function readVersionTracker(spreadsheetId) {
  let values = await readSheetValues(
    spreadsheetId,
    `'Setup'!A${VERSION_TRACKER_DATA_START}:F${VERSION_TRACKER_MAX_ROW}`
  )

  // Check if new location has any version data
  const hasNewData = values.some(row => row && (row[1] || '').toString().trim())

  if (!hasNewData) {
    // Fallback: check old location (rows 74-80) for one-time migration
    try {
      const oldValues = await readSheetValues(
        spreadsheetId,
        `'Setup'!A${OLD_TRACKER_DATA_START}:F${OLD_TRACKER_MAX_ROW}`
      )
      const hasOldData = oldValues.some(row => row && (row[1] || '').toString().trim())

      if (hasOldData) {
        console.log('[version-mgmt] Migrating version tracker from rows 74-80 to rows 201-250')
        const batchData = []
        // Copy old data to new location
        for (let i = 0; i < oldValues.length; i++) {
          if (oldValues[i] && oldValues[i].length > 0 && (oldValues[i][1] || '').toString().trim()) {
            batchData.push({
              range: `'Setup'!A${VERSION_TRACKER_DATA_START + i}:F${VERSION_TRACKER_DATA_START + i}`,
              values: [oldValues[i]],
            })
          }
        }
        // Clear old rows to prevent ghost data
        for (let row = OLD_TRACKER_DATA_START; row <= OLD_TRACKER_MAX_ROW; row++) {
          batchData.push({
            range: `'Setup'!A${row}:F${row}`,
            values: [['', '', '', '', '', '']],
          })
        }
        if (batchData.length > 0) {
          await batchUpdateSheet(spreadsheetId, batchData)
          console.log(`[version-mgmt] Migration complete: moved ${batchData.length - 7} versions, cleared 7 old rows`)
        }
        values = oldValues
      }
    } catch (migrationErr) {
      console.warn('[version-mgmt] Migration check failed (non-fatal):', migrationErr.message)
    }
  }

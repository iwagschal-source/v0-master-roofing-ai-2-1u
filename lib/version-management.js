...
export async function readSetupConfig(spreadsheetId) {
  ...
  return { rows, locationToggles: locationColHasToggle, activeLocationCols }
}
...
export async function hideEmptyRows(spreadsheetId, tabSheetId, setupRows) {
  ...
  console.log(`[version-mgmt] Hidden ${requests.length} rows (no toggles)`)
}
...
export async function hideEmptyColumns(spreadsheetId, tabSheetId, locationToggles) {
  ...
  console.log(`[version-mgmt] Hidden ${requests.length} location columns (no toggles)`)
}

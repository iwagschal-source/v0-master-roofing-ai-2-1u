...
// Version tracker — NEW location (rows 200-250, 50 slots)
const VERSION_TRACKER_HEADER_ROW = 200
const VERSION_TRACKER_DATA_START = 201
const VERSION_TRACKER_MAX_ROW = 250 // 50 version slots

// OLD location (rows 73-80, 7 slots) — used only for migration
const OLD_TRACKER_HEADER_ROW = 73
const OLD_TRACKER_DATA_START = 74
const OLD_TRACKER_MAX_ROW = 80
...
const SETUP_COL = {
  ...
  TOOL_STATUS: 16,  // Q (Bluebeam Tool Status: "Generated" or empty)
}
 ...
 * Checks both existing tab names AND tracker history to avoid reusing deleted names.
 * @param {string[]} existingNames - List of existing tab names
 * @param {string[]} [trackerNames] - List of names from version tracker (includes deleted)
 * @param {string} [dateStr] - Optional date string; defaults to today
 * @returns {string}
 */
export function generateVersionName(existingNames, trackerNames = [], dateStr = null) {
  const allUsed = new Set([...existingNames, ...trackerNames])
  const today = dateStr || new Date().toISOString().split('T')[0]
  if (!allUsed.has(today)) return today

  let suffix = 2
  while (allUsed.has(`${today}-v${suffix}`)) {
    suffix++
  }
...
export async function readSetupConfig(spreadsheetId) {
  ...
  for (let rowIdx = 0; rowIdx < values.length; rowIdx++) {
    ...
    // Read tool name from column P and tool status from column Q
    const toolName = (row[SETUP_COL.TOOL_NAME] || '').toString().trim()
    const toolStatus = (row[SETUP_COL.TOOL_STATUS] || '').toString().trim()
 ...
 * @param {Object} [sectionLocationNames] - From readSetupConfig(), maps section→location name array
 */
export async function transferSetupToVersion(spreadsheetId, versionTabName, setupRows, projectName, sectionLocationNames = null) {
  const batchData = []
  ...
  // Transfer custom location names from Setup section headers to version tab (G-M on header rows)
  if (sectionLocationNames) {
    for (const [headerRow, sectionName] of Object.entries(SECTION_NAME_BY_HEADER_ROW)) {
      const names = sectionLocationNames[sectionName]
      if (names && names.some(n => n)) {
        batchData.push({
          range: `'${versionTabName}'!G${headerRow}:M${headerRow}`,
          values: [names],
        })
      }
    }
...
/**
 * Parse raw sheet values into version objects.
 * @param {Array} values - Raw row data from Google Sheets
 * @param {number} dataStartRow - The first data row number (for calculating absolute row)
 * @returns {Array}
 */
function parseVersionRows(values, dataStartRow) {
  const versions = []
...
/**
 * Expand the Setup tab grid to have at least `minRows` rows.
 * Uses the Sheets API appendDimension request.
 *
 * @param {string} spreadsheetId
 * @param {number} setupSheetId - The numeric sheet ID of the Setup tab
 * @param {number} currentRows - Current number of rows
 * @param {number} minRows - Minimum rows needed
 */
async function expandSetupGrid(spreadsheetId, setupSheetId, currentRows, minRows) {
  if (currentRows >= minRows) return
  const rowsToAdd = minRows - currentRows
  console.log(`[version-mgmt] Expanding Setup grid from ${currentRows} to ${minRows} rows (+${rowsToAdd})`)
  ...
  if (!response.ok) {
    ...
    throw new Error(`Failed to expand Setup grid: ${err}`)
  }
  console.log(`[version-mgmt] Grid expanded to ${minRows} rows`)
}

/**
 * Read the version tracker from the Setup tab.
 * Uses NEW location (rows 201-250) with automatic migration from OLD location (rows 74-80).
 *
 * Migration logic:
 * 1. Try new location first (may fail if grid is too small — that's OK)
 * 2. If new location has data → use it, done
 * 3. If new location empty/unreachable → read old location (rows 74-80)
 * 4. If old has data → expand grid, copy to new location, verify, then clear old
 * 5. If both empty → return empty array
 *
 * @param {string} spreadsheetId
...
export async function readVersionTracker(spreadsheetId) {
  // Step 1: Try new location (rows 201-250)
  let newVersions = []
  let gridTooSmall = false
  try {
    const newValues = await readSheetValues(
      spreadsheetId,
      `'Setup'!A${VERSION_TRACKER_DATA_START}:F${VERSION_TRACKER_MAX_ROW}`
    )
    newVersions = parseVersionRows(newValues, VERSION_TRACKER_DATA_START)
  } catch (err) {
    if (err.message && err.message.includes('exceeds grid limits')) {
      gridTooSmall = true
      console.log('[version-mgmt] New location exceeds grid — old project, will check old location')
    } else {
  ...
  if (newVersions.length > 0) {
    // New location has data — use it
    return newVersions
  }

  // Step 2: New location empty or unreachable — check old location (rows 74-80)
  console.log('[version-mgmt] Checking old tracker location (rows 74-80)...')
  const oldValues = await readSheetValues(
    spreadsheetId,
    `'Setup'!A${OLD_TRACKER_DATA_START}:F${OLD_TRACKER_MAX_ROW}`
  )
  const oldVersions = parseVersionRows(oldValues, OLD_TRACKER_DATA_START)

  if (oldVersions.length === 0) {
    // Both empty — no versions yet
    console.log('[version-mgmt] No versions in old or new location')
    return []
  }

  // Step 3: Migrate old → new
  // First, expand the grid if needed
  if (gridTooSmall) {
    const tabs = await getSpreadsheetTabs(spreadsheetId)
    const setupTab = tabs.find(t => t.title === 'Setup')
    if (setupTab) {
      // Grid properties may not be available, so use a safe default
      await expandSetupGrid(spreadsheetId, setupTab.sheetId, 80, VERSION_TRACKER_MAX_ROW + 1)
    }
  }

  console.log(`[version-mgmt] Migrating ${oldVersions.length} versions from old location to new (rows 201+)...`)
  const migrateBatch = []
  // Write header row (row 200)
  migrateBatch.push({
    range: `'Setup'!A${VERSION_TRACKER_HEADER_ROW}:F${VERSION_TRACKER_HEADER_ROW}`,
    values: [['Active', 'Sheet Name', 'Created Date', 'Items Count', 'Locations Count', 'Status']],
  })
  for (let i = 0; i < oldVersions.length; i++) {
    const v = oldVersions[i]
    const newRow = VERSION_TRACKER_DATA_START + i
    migrateBatch.push({
      range: `'Setup'!A${newRow}:F${newRow}`,
      values: [[v.active, v.sheetName, v.created, v.itemsCount || '', v.locationsCount || '', v.status]],
    })
  }
  await batchUpdateSheet(spreadsheetId, migrateBatch)
  console.log(`[version-mgmt] Wrote ${oldVersions.length} versions to new location`)

  // Step 4: Verify the write succeeded by reading back
  const verifyValues = await readSheetValues(
    spreadsheetId,
    `'Setup'!A${VERSION_TRACKER_DATA_START}:F${VERSION_TRACKER_DATA_START + oldVersions.length - 1}`
  )
  const verifiedVersions = parseVersionRows(verifyValues, VERSION_TRACKER_DATA_START)

  if (verifiedVersions.length !== oldVersions.length) {
    console.error(`[version-mgmt] Migration verification FAILED: wrote ${oldVersions.length} but read back ${verifiedVersions.length}. Keeping old data intact.`)
    // Return old versions with their old row numbers — don't clear anything
    return oldVersions
  }

  // Step 5: Verified — now clear old location
  console.log('[version-mgmt] Migration verified. Clearing old location (rows 74-80)...')
  const clearBatch = []
  for (let r = OLD_TRACKER_DATA_START; r <= OLD_TRACKER_MAX_ROW; r++) {
    clearBatch.push({
      range: `'Setup'!A${r}:F${r}`,
      values: [['', '', '', '', '', '']],
    })
  }
  await batchUpdateSheet(spreadsheetId, clearBatch)
  console.log('[version-mgmt] Old location cleared. Migration complete.')
...
export async function addVersionTrackerEntry(spreadsheetId, sheetName, itemsCount, locationsCount) {
  ...
  // Ensure the grid is large enough for the new tracker location
  // (readVersionTracker expands during migration, but if both old/new were empty, grid may still be small)
  try {
    await readSheetValues(spreadsheetId, `'Setup'!A${VERSION_TRACKER_DATA_START}:A${VERSION_TRACKER_DATA_START}`)
  } catch (err) {
    if (err.message && err.message.includes('exceeds grid limits')) {
      console.log('[version-mgmt] Grid too small for new tracker location — expanding...')
      const tabs = await getSpreadsheetTabs(spreadsheetId)
      const setupTab = tabs.find(t => t.title === 'Setup')
      if (setupTab) {
        await expandSetupGrid(spreadsheetId, setupTab.sheetId, 80, VERSION_TRACKER_MAX_ROW + 1)
  ...
  if (newRow > VERSION_TRACKER_MAX_ROW) {
    console.warn('[version-mgmt] Version tracker full (max 50 versions). Overwriting last row.')
...
export async function copyExistingVersion(spreadsheetId, sourceSheetName) {
  ...
  const newName = generateVersionName(existingNames, trackerNames)
...
export async function deleteVersion(spreadsheetId, sheetName, force = false) {
  ...
  // Fully remove from version tracker — clear the row so the slot is reusable
  const versions = await readVersionTracker(spreadsheetId)
  const version = versions.find(v => v.sheetName === sheetName)
  if (version) {
    await batchUpdateSheet(spreadsheetId, [{
    ...
    console.log(`[version-mgmt] Cleared tracker row ${version.row} for "${sheetName}"`)
  }

  // If deleted version was active, activate the most recent remaining version
  if (version?.active) {
    const remaining = versions.filter(v => v.sheetName !== sheetName && v.sheetName)
    if (remaining.length > 0) {
      const latest = remaining[remaining.length - 1]
      await batchUpdateSheet(spreadsheetId, [{
        range: `'Setup'!A${latest.row}`,
        values: [[true]],
      }])
      console.log(`[version-mgmt] Auto-activated "${latest.sheetName}" after deletion`)
    }
  }
...
/**
 * Update Column Q (Tool Status) on the Setup tab after BTX generation (11E)
 * Marks each generated item row with "Generated" and a timestamp.
 *
 * @param {string} spreadsheetId
 * @param {Array<{itemId: string, rowNum: number}>} generatedItems — items that were sent to Python
 */
export async function updateToolStatus(spreadsheetId, generatedItems) {
  if (!generatedItems?.length) return

  const timestamp = new Date().toISOString().split('T')[0]
  const updates = generatedItems.map(item => ({
    range: `'Setup'!Q${item.rowNum}`,
    values: [[`Generated ${timestamp}`]],
  }))

  await batchUpdateSheet(spreadsheetId, updates)
  console.log(`[version-mgmt] Marked ${generatedItems.length} items as Generated in Column Q`)
}

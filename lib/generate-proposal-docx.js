...
export function generateProposalDocx(templateBuffer, data) {
  ...
  // Prepare line items data - template has 8 slots
  const itemsData = {}
  for (let i = 1; i <= 8; i++) {
    const item = data.lineItems?.[i - 1]
    if (item) {
      itemsData[`item_${i}_title`] = item.title || ""
      itemsData[`item_${i}_description`] = item.description || ""
      itemsData[`item_${i}_price`] = formatCurrency(item.amount || 0)
    } else {
      // Empty placeholders for unused slots
      itemsData[`item_${i}_title`] = ""
      itemsData[`item_${i}_description`] = ""
      itemsData[`item_${i}_price`] = ""
    }
  }

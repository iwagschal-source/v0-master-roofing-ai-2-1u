...
// ============================================================================
// STANDALONE TAKEOFF SPREADSHEET (PREFERRED - Step 8.B.6)
// Each project gets its own Google Sheets workbook
// Future versions = tabs within that workbook (date-based)
// ============================================================================
 ...
 * Copies the template to a new workbook (not a tab in existing sheet)
 *
 * @param {string} projectId - The project ID
 * @param {string} projectName - Human-readable project name
 * @returns {Promise<{spreadsheetId: string, spreadsheetUrl: string, embedUrl: string}>}
 */
export async function createProjectTakeoffSheet(projectId, projectName) {
  const accessToken = await getAccessToken()

  const datestamp = new Date().toISOString().split('T')[0] // 2026-01-29
  const sheetName = `Takeoff - ${projectName || projectId} - ${datestamp}`

  // Copy entire template workbook to new standalone spreadsheet
  const copyResponse = await fetch(
...
export async function populateTakeoffSheet(spreadsheetId, columns, lineItems) {
  ...
  // Row 1: Header row with location names (starting at column C)
  // Column A = Unit Cost, B = Scope, C onwards = locations
  const headerRow = columns.map((col, idx) => col.name || `Location ${idx + 1}`)
  data.push({
    range: `'Sheet1'!C1:${String.fromCharCode(67 + columns.length - 1)}1`,
    values: [headerRow]
  })

  // Rows 2+: Line items (scope names in column B)
  if (lineItems.length > 0) {
    const scopeNames = lineItems.map(item => [item.scope_name || item.scope_code])
    data.push({
      range: `'Sheet1'!B2:B${lineItems.length + 1}`,
      values: scopeNames
    })
  }

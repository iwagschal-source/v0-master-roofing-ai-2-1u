...
export async function createProjectTakeoffSheet(projectId, projectName, existingFolderId = null) {
  ...
  try {
    const takeoffTabName = await getActiveSheetName(newSpreadsheetId)
    const nameRange = encodeURIComponent(`'${takeoffTabName}'!A2`)
    const nameResponse = await fetch(
      `https://sheets.googleapis.com/v4/spreadsheets/${newSpreadsheetId}/values/${nameRange}?valueInputOption=RAW`,
      {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${accessToken}`,
    ...
    if (nameResponse.ok) {
      console.log(`[Sheets] Wrote project name to '${takeoffTabName}'!A2: ${projectName || projectId}`)
    } else {

...
export async function getActiveSheetName(spreadsheetId) {
  ...
  // Fallback: return "DATE" for backward compat with old single-tab projects
  return 'DATE'
}
...
export async function createProjectTakeoffSheet(projectId, projectName, existingFolderId = null) {
  ...
  // Post-copy: Rename "DATE" tab to YYYY-MM-DD and write project name to both tabs
  let versionTabName = datestamp // e.g., "2026-02-13"
  try {
    // Fetch spreadsheet metadata to find the "DATE" tab's sheetId
    ...
    if (metaResponse.ok) {
      ...
      if (dateTab) {
            ...
            body: JSON.stringify({
              requests: [{
                updateSheetProperties: {
                  properties: {
                    sheetId: dateTab.properties.sheetId,
                    title: datestamp,
                  },
                  fields: 'title',
                },
        ...
        if (renameResponse.ok) {
          console.log(`[Sheets] Renamed "DATE" tab to "${datestamp}"`)
        } else {
          console.warn('[Sheets] Failed to rename DATE tab:', await renameResponse.text())
          versionTabName = 'DATE' // fallback to original name
        }
      } else {
        // No "DATE" tab found — use getActiveSheetName as fallback
        versionTabName = await getActiveSheetName(newSpreadsheetId)
        console.log(`[Sheets] No "DATE" tab found, using active tab: "${versionTabName}"`)
      }
    }
  } catch (err) {
    console.warn('[Sheets] Tab rename failed:', err.message)
    versionTabName = 'DATE' // fallback
  }

  // Write project name to BOTH Setup tab and version tab row 2
  const projectDisplayName = projectName || projectId
  try {
    const batchData = [
      { range: `'Setup'!A2`, values: [[projectDisplayName]] },
      { range: `'${versionTabName}'!A2`, values: [[projectDisplayName]] },
    ]
    ...
    if (batchWriteResponse.ok) {
      console.log(`[Sheets] Wrote project name to Setup!A2 and '${versionTabName}'!A2: ${projectDisplayName}`)
    } else {
      console.warn('[Sheets] Failed to write project name:', await batchWriteResponse.text())
    }
  } catch (err) {

...
export async function fillBluebeamDataToSpreadsheet(spreadsheetId, bluebeamItems, config = null) {
  ...
  // Read all three header rows in parallel (full row for dynamic column detection)
  // TODO: Rows 45/54 are hardcoded — should detect section headers dynamically like preview/route.js does
  const headerRanges = [
    `'${sheetName}'!A3:Z3`,   // ROOFING header
    `'${sheetName}'!A45:Z45`, // BALCONIES header
    `'${sheetName}'!A54:Z54`  // EXTERIOR header
  ]
  ...
  const mergedLocationMap = {
    ...sectionLocationMaps.ROOFING,
    ...sectionLocationMaps.BALCONIES,
    ...sectionLocationMaps.EXTERIOR,
  }

  console.log('[fillBluebeamData] Section location maps:', JSON.stringify(sectionLocationMaps, null, 2))
  console.log('[fillBluebeamData] Merged location map:', JSON.stringify(mergedLocationMap, null, 2))

  // Also read Column A to get item_ids for matching
  const colAResponse = await fetch(
    `https://sheets.googleapis.com/v4/spreadsheets/${spreadsheetId}/values/${encodeURIComponent(`'${sheetName}'!A4:A200`)}`,
    {
      headers: {
        'Authorization': `Bearer ${accessToken}`,
      },
  ...
  // Build code → row mapping purely from Column A scan.
  // NOT seeded with ITEM_ID_TO_ROW — on modified sheets (deleted rows), the
  // hardcoded template row numbers are wrong and would write data to wrong cells.
  const codeRows = {}
  if (colAResponse.ok) {

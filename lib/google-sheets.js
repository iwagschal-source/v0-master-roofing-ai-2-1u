...
// Project folder structure: KO Projects > [Project] > Drawings, Bluebeam, Takeoff, Markups, Proposals
// ============================================================================
 ...
 * Structure: KO Projects > [Project Name] - [ID] > Drawings, Bluebeam, Takeoff, Markups, Proposals
 *
 * @param {string} projectId - The project ID
 * @param {string} projectName - Human-readable project name
 * @param {string} [existingFolderId] - Existing folder ID from BigQuery (skip creation if exists)
 * @returns {Promise<{folderId: string, drawingsFolderId: string, bluebeamFolderId: string, takeoffFolderId: string, markupsFolderId: string, proposalsFolderId: string}>}
 */
export async function getOrCreateProjectFolder(projectId, projectName, existingFolderId = null) {
  ...
  if (existingFolderId) {
    return { folderId: existingFolderId, drawingsFolderId: null, bluebeamFolderId: null, takeoffFolderId: null, markupsFolderId: null, proposalsFolderId: null }
  }
  ...
  // Create standard subfolders (Phase 12: all 5 category folders)
  const subfolderNames = ['Drawings', 'Bluebeam', 'Takeoff', 'Markups', 'Proposals']
  const subfolderIds = {}
  ...
  return {
    ...
    bluebeamFolderId: subfolderIds.bluebeam || null,
    takeoffFolderId: subfolderIds.takeoff || null,
...
/**
 * Refresh the Library tab on a spreadsheet from BigQuery v_library_complete.
 * Used after adding items and during project creation.
 * @param {string} spreadsheetId - The spreadsheet to refresh
 * @param {string} [accessToken] - Optional; fetched if not provided
 * @returns {Promise<{success: boolean, count: number}>}
 */
export async function refreshLibraryTab(spreadsheetId, accessToken = null) {
  ...
  if (libraryRows.length === 0) return { success: false, count: 0 }
  ...
  if (writeResponse.ok) {
    console.log(`[Sheets] Refreshed Library tab with ${libraryRows.length} items on ${spreadsheetId}`)
    return { success: true, count: libraryRows.length }
  } else {
...
export async function createProjectTakeoffSheet(projectId, projectName, existingFolderId = null) {
  ...
  try {
    await refreshLibraryTab(newSpreadsheetId, accessToken)
  } catch (err) {
 ...
 */
export async function fillBluebeamDataToSpreadsheet(spreadsheetId, bluebeamItems, config = null, sheetName = null, validTargets = null) {
  const accessToken = await getAccessToken()
  if (!sheetName) {
  ...
  // Helper: convert column letter to 0-based location toggle index (G=0, H=1, ..., M=6)
  const colLetterToToggleIndex = (letter) => {
    const colIdx = letter.charCodeAt(0) - 65 // A=0, G=6, M=12
    return colIdx - 6 // G→0, H→1, ..., M→6
  }
  ...
  for (const item of bluebeamItems) {
    ...
    // Setup config filter: skip items not toggled in Setup tab
    if (validTargets && !validTargets.has(code)) {
      details.push({ code, floor, quantity: item.quantity, row, status: 'ITEM_NOT_ACTIVE' })
      continue
    }
    ...
    let resolvedCol = locationMap[floor] || mergedLocationMap[floor]

    if (!resolvedCol) {
      // Try some common normalizations
      const normalizedFloor = floor?.replace(/[^A-Z0-9]/g, '')
      resolvedCol = locationMap[normalizedFloor] || mergedLocationMap[normalizedFloor]
    ...
    // Setup config filter: skip locations not toggled for this item
    if (validTargets) {
      const toggleIdx = colLetterToToggleIndex(resolvedCol)
      if (toggleIdx >= 0 && toggleIdx <= 6 && !validTargets.get(code)?.has(toggleIdx)) {
        details.push({ code, floor, quantity: item.quantity, row, section, col: resolvedCol, status: 'LOCATION_NOT_ACTIVE' })
        continue
      }
    }

    const isNormalized = !(locationMap[floor] || mergedLocationMap[floor])
    const detailIdx = details.length
    details.push({ code, floor, quantity: item.quantity, row, section, col: resolvedCol, status: isNormalized ? 'MATCHED_NORMALIZED' : 'MATCHED', previousValue: 0, accumulatedTotal: item.quantity })
    matchedCells.push({ range: `'${sheetName}'!${resolvedCol}${row}`, quantity: item.quantity, detailIndex: detailIdx })
  }
  ...
  // Deduplicate matchedCells by range — sum quantities for same cell (e.g., "5THFLOOR" + "5THFLOOR (1)")
  const dedupMap = {}
  for (const mc of matchedCells) {
    if (dedupMap[mc.range]) {
      dedupMap[mc.range].quantity += mc.quantity
      // Update the detail entry to reflect combined quantity
      const detail = details[mc.detailIndex]
      if (detail) detail.accumulatedTotal = dedupMap[mc.range].quantity
    } else {
      dedupMap[mc.range] = { ...mc }
    }
  }
  const dedupedCells = Object.values(dedupMap)

  // ACCUMULATION: Batch-read existing values at all target cells
  const readRanges = dedupedCells.map(mc => mc.range)

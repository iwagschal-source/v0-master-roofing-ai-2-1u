...
export async function fillBluebeamDataToSpreadsheet(spreadsheetId, bluebeamItems, config = null) {
  ...
  // Read existing scope names from column C to find row numbers
  // Scopes are in column C starting at row 4
  ...
  if (scopesResponse.ok) {
    ...
    const existingScopes = scopesData.values || []
    existingScopes.forEach((row, idx) => {
      const scopeName = row[0]?.toString().toUpperCase().trim()
      if (scopeName) {
        codeRows[scopeName] = idx + 4 // Row 4 is index 0
      }
    })
  }

  console.log('[fillBluebeamData] Location columns:', locationColumns)
  ...
  for (const item of bluebeamItems) {
    ...
    if (row && col) {
      ...
      console.log(`[fillBluebeamData] Mapping ${code} @ ${floor} -> ${col}${row} = ${item.quantity}`)
    } else {
      console.log(`[fillBluebeamData] No match for ${code} @ ${floor} (row=${row}, col=${col})`)
    }
  }

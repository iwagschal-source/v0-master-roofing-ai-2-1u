...
export async function createProjectTakeoffSheet(projectId, projectName, existingFolderId = null) {
  ...
  // Post-copy: Write project name to row 2 (replaces {PROJECT_NAME} placeholder)
  try {
    const nameResponse = await fetch(
      `https://sheets.googleapis.com/v4/spreadsheets/${newSpreadsheetId}/values/${encodeURIComponent("'DATE'!A2")}?valueInputOption=RAW`,
      {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${accessToken}`,
        ...
        body: JSON.stringify({ values: [[projectName || projectId]] }),
      }
    )
    if (nameResponse.ok) {
      console.log(`[Sheets] Wrote project name to row 2: ${projectName || projectId}`)
    } else {
      console.warn('[Sheets] Failed to write project name:', await nameResponse.text())
    }
  } catch (err) {
  ...
  try {
    ...
    if (libraryRows.length > 0) {
      ...
      const range = encodeURIComponent(`'Library'!A1:AD${allValues.length}`)
...
const TEMPLATE_SECTIONS = {
  ROOFING: { headerRow: 3, startRow: 4, endRow: 35 },
  WATERPROOFING: { headerRow: 36, startRow: 37, endRow: 39 },
  BALCONIES: { headerRow: 40, startRow: 41, endRow: 45 },
  EXTERIOR: { headerRow: 49, startRow: 50, endRow: 67 }
}

// item_id → row mapping (matches template structure)
// From scripts/populate-template-item-ids.js
const ITEM_ID_TO_ROW = {
  // ROOFING SECTION (rows 4-35)
  'MR-001VB': 4,
  'MR-002PITCH': 5,
  'MR-003BU2PLY': 6,
  'MR-004UO': 7,
  ...
  'MR-030GREEN': 33,
  'MR-031FLASHGRN': 34,
  // WATERPROOFING SECTION (rows 37-39)
  'MR-032RECESSWP': 37,
  'MR-FIRE-LIQ': 38,
  'MR-THORO': 39,
  // BALCONIES SECTION (rows 41-44)
  'MR-033TRAFFIC': 41,
  'MR-034DRIP': 42,
  'MR-036DOORBAL': 43,
  'MR-035LFLASH': 44,
  // EXTERIOR SECTION (rows 50-67)
  'MR-037BRICKWP': 50,
  'MR-038OPNBRKEA': 51,
  'MR-039OPNBRKLF': 52,
  'MR-040PANELWP': 54,

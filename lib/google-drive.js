...
export async function listFilesInFolder(accessToken, folderId) {
  ...
  const fields = 'files(id,name,mimeType,size,modifiedTime,webViewLink,webContentLink,shortcutDetails)'
  const res = await fetch(
  ...
  // Ensure all listed files have public read permission (for Drive preview iframe)
  // Run in background — don't block the response
  if (files.length > 0) {
    Promise.all(files.map(f => setFilePublicRead(accessToken, f.id))).catch(() => {})
  }
...
export async function uploadFileToDrive(accessToken, folderId, fileName, fileBuffer, mimeType) {
  ...
  // Set file to "anyone with the link can view" so Drive preview iframe works
  if (result?.id) {
...
/**
 * Set a Drive file to "anyone with the link can view"
 * Required for Drive preview iframe to work without user authentication
 */
export async function setFilePublicRead(accessToken, fileId) {
  try {
    const res = await fetch(
    ...
    if (!res.ok) {
      console.warn('[Drive] Failed to set public permission:', await res.text())
    }
  } catch (err) {
    console.warn('[Drive] Permission set failed (non-fatal):', err.message)
  }
}

/**
 * Create a Drive shortcut to a target file inside a folder.
 * Checks if shortcut already exists before creating a new one.
 * Returns the shortcut file id or null.
 */
export async function createDriveShortcut(accessToken, parentFolderId, targetFileId, shortcutName) {
  try {
    // Check if a shortcut to this target already exists in the folder
    const q = `'${parentFolderId}' in parents and mimeType='application/vnd.google-apps.shortcut' and trashed=false`
    const searchRes = await fetch(
      `${DRIVE_API}/files?q=${encodeURIComponent(q)}&fields=files(id,shortcutDetails)`,
      { headers: { 'Authorization': `Bearer ${accessToken}` } }
    )
    if (searchRes.ok) {
      const data = await searchRes.json()
      const existing = (data.files || []).find(f => f.shortcutDetails?.targetId === targetFileId)
    ...
    if (createRes.ok) {
      const shortcut = await createRes.json()
      console.log(`[Drive] Created shortcut: ${shortcutName} → ${targetFileId}`)
      return shortcut.id
    }
    return null
  } catch (err) {
    console.warn('[Drive] Shortcut creation failed (non-fatal):', err.message)
    return null
  }
}

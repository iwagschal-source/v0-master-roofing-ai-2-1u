...
// Template path
const TEMPLATE_PATH = join(process.cwd(), 'templates', 'Proposal_Template_v1_FIXED.docx')
...
export async function POST(request, { params }) {
  try {
    ...
    const previewData = await getProposalPreviewData(projectId)
    ...
    const templateContent = readFileSync(TEMPLATE_PATH)
 ...
 */
async function getProposalPreviewData(projectId) {
  try {
    // Get project info
    const projectResult = await runQuery(
      `SELECT
       ...
       LEFT JOIN \`master-roofing-intelligence.mr_main.contacts\` c
    ...
    const previewRes = await fetch(`${baseUrl}/api/ko/proposal/${projectId}/preview`, {
      method: 'GET',
